...
## 2.1. Сопоставление с мастера настройки плагина (скриншоты 1–6)

По скриншотам плагина **virtual-hdm-for-taxservice-am** (assets/screenshot-1.png … screenshot-6.png) — что есть на каждом шаге и что из этого нужно для Next.js.

| Шаг | Название (скриншот) | Данные на экране | У нас в .env / проекте | Примечание |
|-----|----------------------|------------------|------------------------|------------|
| **1** | Նույնականացում (Идентификация) | TAX-SER (ключ/лицензия), срок действия | — | Лицензия плагина HK Agency; для API ПЕК не нужна. Мы используем .crt + .key. |
| **2** | Կազմակերպության Նույնականացում | ՀՎՀՀ (TIN), «Էլ. ՀԴՄ գրանցման համար» (CRN), загрузка .jks | **EHDM_TIN**, **EHDM_CRN**; сертификат в Private/ (.crt+.key) | У нас PEM (.crt/.key), не JKS. Данные совпадают. |
| **3** | Հարկման կարգավորում (Налог) | Тип: ԱԱՀ-ով հարկվող; Բաժին 1-1; ԱԱՀ 16.67%; Գանձապահ 1; чекбоксы (SKU↔код и т.д.) | **EHDM_DEP=1**, **EHDM_CASHIER_ID=1** | dep=1 — «с НДС». VAT % при необходимости считаем в коде. |
| **4** | Ապրանքների կարգավորում (Товары) | ԱՏԳ կոդ: **0901** (в примере); единица по умолчанию **Հատ**; доставка: АТГ **49.42**, код **007**, единица Հատ | **EHDM_DEFAULT_ADG_CODE=2201**, **EHDM_DEFAULT_UNIT=Հատ** | В скриншоте 0901 — у вас в .env 2201 (по вашему выбору). Для строки «доставка» в чеке позже: отдельный код АТГ и goodCode (напр. 49.42 и 007). |
| **5** | Կտրոնի տպման կարգավորում (Печать чека) | Шлюз оплаты (ACBA/Idram и т.д.), автопечать при статусе «Processing» | — | В Next.js: вызывать print после успешной оплаты (webhook/проверка статуса). |
| **6** | Пример чека в письме | TIN, CRN, SN, Fiscal, ԱՏԳ 0901, Գանձապահ 1, Բաժին 1-1 | Все реквизиты совпадают с нашими | Подтверждает: те же TIN/CRN/сертификат; в чеке виден adgCode по позиции и cashier. |

**Итого что учли и добавили:**  
CRN, TIN, сертификат, ключ, passphrase, seq, код АТГ по умолчанию (2201), **dep** (1), **единица по умолчанию** (Հատ), **cashierId** (1). Для доставки отдельной строкой — при реализации добавить код АТГ доставки и goodCode доставки (из шага 4 плагина или свои).
