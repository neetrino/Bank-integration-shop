...
## 0. Важные вопросы: тест на реальном окружении и домен

### Можно ли тестировать сразу на реальном проекте (без тестового стенда)?

**Да.** У вас нет тестового окружения ПЕК (тестовый URL в документации: `https://10.3.14.123:447`, тестовый CRN/сертификат обычно запрашивают отдельно). У вас уже есть **реальный** CRN и сертификат — значит вы работаете с **production API** (`https://ecrm.taxservice.am/...`).

- **Тестировать можно сразу на реальном API.** Каждый успешный вызов **print** будет создавать **реальный фискальный чек** в налоговой (с реальным receiptId, fiscal и т.д.). Откатить или «стереть» его нельзя.
- Рекомендация: первые проверки делать с **минимальными суммами** или одним тестовым заказом, чтобы убедиться в формате и логике; после этого переходить к обычным продажам.

### Нужно ли регистрировать домен у ПЕК? Можно ли отправлять invoice с другого домена?

**Нет, домен регистрировать в ПЕК для E-HDM не нужно.** И отправлять запросы (в т.ч. print) можно с **любого домена/сервера**.

- **E-HDM** — это **сервер-к-серверу** запросы: ваш бэкенд (Next.js) сам вызывает API ПЕК по HTTPS. Налоговая проверяет только **клиентский сертификат** и **CRN** в теле запроса. Откуда идёт запрос (какой домен, localhost, staging, production) — **не проверяется**.
- То есть: можно разрабатывать на localhost, деплоить на любой домен (например `shop.example.com` или другой) — для ПЕК это не важно. Важно только: на сервере должны быть правильные .crt, .key и CRN в .env.

**Сравнение с банками:** у Idram/Ameria нужно **регистрировать callback-URL** (RESULT_URL, SUCCESS_URL, FAIL_URL и т.д.) — там **домен/URL обязателен**, банк шлёт запросы на ваш зарегистрированный адрес. Для E-HDM запрос идёт **от вас к ПЕК**, а не наоборот, поэтому привязки к домену нет.
