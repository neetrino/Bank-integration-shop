...
add_action('woocommerce_thankyou', 'hkdigital_woocomerceShowErrorMessageIDbank', 4);
if (!function_exists('hkdigital_woocomerceShowErrorMessageIDbank')) {
    /**
     ...
     * @return bool
     */
    function hkdigital_woocomerceShowErrorMessageIDbank($order_id)
    {
        $order = wc_get_order($order_id);
        if ($order->has_status('failed')) {
            $orderFailedMessage = get_post_meta($order_id, 'FailedMessageIDBank', true);
